    ...
    <>
      ...
      <div className='contenedor'>
        {medicamentos.map((categoria, posicion) => (
          <table className={`table-style ${posicion % 2 === 0 ? 'fila-par' : 'fila-impar'}`} key={posicion}>
            {posicion === 0 && (
              <tr>
                <th></th>
            ...
            {Array.from({ length: 4 }).map((_, index) => (
              <tr key={index}>
                {index === 0 && (
                  <td
                    rowSpan={4}
                    className={`columan-fija icons ${categoria.tiempo.toLowerCase()}`}
                  >
                    <div className='icons'>
